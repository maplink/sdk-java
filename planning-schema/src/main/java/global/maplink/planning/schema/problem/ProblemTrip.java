...
public class ProblemTrip {
    ...
    private void validateWithPoints(List<Site> sites, List<Site> depots) {
        var points = Stream.concat(sites.stream(), depots.stream())
                .count();
    ...
    private boolean checkPointsEquality(List<Site> sites, List<Site> depots) {
        var allCoordinates = Stream.concat(
                        sites.stream().map(Site::getCoordinates),
                        depots.stream().map(Site::getCoordinates))
